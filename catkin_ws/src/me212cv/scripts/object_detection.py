...
# Publisher for camera pose estimate from the red/green/blue object
red_pub = rospy.Publisher('red', Pose, queue_size=10)
green_pub = rospy.Publisher('green', Pose, queue_size=10)
blue_pub = rospy.Publisher('blue', Pose, queue_size=10)
...
def HSVObjectDetection(cv_image, toPrint = True, color='red'):
    ...
    if color == 'red':
        ...
        lower = np.array([110, 60, 35])     #[220, 25, 15]
        upper = np.array([120, 195, 220])   #[240, 75, 85]
    elif color == 'green':
        lower = np.array([50, 50, 35])      #[100, 20, 15]
        upper = np.array([90, 210, 210])    #[180, 80, 70]
...
def rosRGBDCallBack(rgb_data, depth_data):
    ...
    red_contours, mask_image = HSVObjectDetection(cv_image, toPrint = False)
    red_contours.sort(key=cv2.contourArea, reverse=True)
    
    green_contours, mask_image = HSVObjectDetection(cv_image, toPrint = False, color='green')
    green_contours.sort(key=cv2.contourArea, reverse=True)
    
    blue_contours, mask_image = HSVObjectDetection(cv_image, toPrint = False, color='blue')
    blue_contours.sort(key=cv2.contourArea, reverse=True)
    
    color_contours = [red_contours, green_contours, blue_contours]
    
    max = True
    
    for i in range(3):
        #~ print i, len(color_contours[i])
        for cnt in color_contours[i]:
            if max:
                xp,yp,w,h = cv2.boundingRect(cnt)
                centerx, centery = xp+w/2, yp+h/2
                ...
                #~ showPyramid(centerx, centery, zc, w, h)
                (xc, yc, zc) = getXYZ(centerx,centery,zc,fx,fy,cx,cy)
                ...
                if i == 0:
                    red_pub.publish(pose)
                    cv2.putText(cv_image,'red',(centerx-w/4,centery), font, 0.5, (0,255,255), 2, cv2.LINE_AA)
                elif i == 1:
                    green_pub.publish(pose)
                    cv2.putText(cv_image,'green',(centerx-w/4,centery), font, 0.5, (0,255,255), 2, cv2.LINE_AA)
                elif i == 2:
                    blue_pub.publish(pose)
                    cv2.putText(cv_image,'blue',(centerx-w/4,centery), font, 0.5, (0,255,255), 2, cv2.LINE_AA)
